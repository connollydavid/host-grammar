...
// Punctuation pivots are out of scope: the word tokenizer drops punctuation, so a
// dash entry would never match (the dash already earns a decoration tell). "than"
// is excluded so a comparison ("no faster than X") is not read as antithesis.
const NEGATIONS: &[&str] = &["not", "isn't", "isnt", "doesn't", "doesnt", "won't", "wont", "never", "no"];
const PIVOTS: &[&str] = &["it's", "its", "but", "rather", "instead"];
    ...
    for lex in CORPUS {
        for phrase in lex.phrases {
            ...
            while let Some(rel) = lower[from..].find(phrase) {
                ...
                // Word boundaries on the adjacent character, not the adjacent byte:
                // a non-ASCII letter abutting the phrase must read as part of a word.
                let left = lower[..at].chars().next_back().is_none_or(|c| !c.is_alphanumeric());
                let right = lower[end..].chars().next().is_none_or(|c| !c.is_alphanumeric());
...
// comparable. is_triad(s) is true when the sentence has exactly three comma-spans,
// an "and"/"or" before the final item, and each span is 1..=5 words (triad_max_span)
// with no internal terminal punctuation. A list of four or more items is an
// ordinary enumeration, not a tricolon.
...
fn anaphora_runs(metas: &[Meta]) -> Vec<(String, usize)> {
    ...
    let mut len = 0usize;
    // A run whose shared opener is an ordinal is a listicle, not an anaphora: the
    // spec classifies a run by ordinal_led into exactly one of the two, and anaphora
    // requires not-ordinal-led. Suppressing it here keeps the two from co-firing.
    let flush = |runs: &mut Vec<(String, usize)>, opener: Option<String>, len: usize| match opener {
        Some(o) if len >= 3 && !ORDINALS.contains(&o.as_str()) => runs.push((o, len)),
        _ => {}
    };
    for m in metas {
        match &m.opener {
            ...
            other => {
                flush(&mut runs, cur.take(), len);
                cur = other.clone();
                len = 1;
            }
        }
    }
    flush(&mut runs, cur, len);
    runs
}
...
// over ORDINALS. Each maximal ordinal run of length >= 3 is one listicle tell
// (mirroring the per-run spec rule), scored like anaphora.
fn listicle_runs(metas: &[Meta]) -> Vec<usize> {
    let mut runs = Vec::new();
    let mut run = 0usize;
    let flush = |runs: &mut Vec<usize>, run: usize| {
        if run >= 3 {
            runs.push(run);
        }
    };
...
fn run_tells(metas: &[Meta], out: &mut Vec<Tell>) {
    ...
    for len in listicle_runs(metas) {
        out.push(Tell {
            id: "listicle",
            kind: Kind::Structural,
            excerpt: format!("{len}-sentence ordinal-led run"),
            weight: ((len - 2) * (len - 2)) as f32,
...
pub fn scan_chunked(text: &str, k: usize) -> Vec<Tell> {
    ...
    let chunk = sents.len().div_ceil(k);
...
// Parse markdown into prose blocks, dropping code blocks and inline code, quoted
// (blockquote) text, and image alt text, and keeping only link visible text (not
// URLs).
fn parse_markdown(md: &str) -> Vec<Block> {
    ...
    for ev in Parser::new(md) {
        match ev {
            ...
            // Quoted text is a citation, not the author's prose: excluded like code.
            Event::Start(Tag::BlockQuote(_)) => quote_depth += 1,
            Event::End(TagEnd::BlockQuote(_)) => quote_depth = quote_depth.saturating_sub(1),
            // Image alt text describes a picture; like a URL it is not prose.
            ...
            // A paragraph inside a list item continues the item; separate the two
            // paragraphs with a break so their boundary words do not fuse into one
            // token. Any other paragraph opens a new prose block.
            Event::Start(Tag::Paragraph) => match cur.as_mut() {
                Some(b) if b.kind == BlockKind::ListItem => {
                    if !b.text.is_empty() {
                        b.text.push_str("\n\n");
                    }
                }
                _ => start(&mut cur, &mut blocks, BlockKind::Para),
            },
            Event::End(TagEnd::Paragraph)
                if cur.as_ref().is_some_and(|b| b.kind == BlockKind::Para) =>
            {
                flush(&mut cur, &mut blocks)
            }
            Event::Start(Tag::Strong) => {
                if let Some(b) =
                    cur.as_mut().filter(|b| b.kind == BlockKind::ListItem && b.text.trim().is_empty())
                {
                    b.bold_first = true;
                }
            }
            Event::Text(t) if !in_code && !in_image && quote_depth == 0 => {
            ...
            // Inline `code`, link/image URLs, quoted text, and html are not prose.
            _ => {}
        }
    }
    flush(&mut cur, &mut blocks);
...
// The non-heading body text, the population the per-sentence and run equations and
// the density score run over.
fn markdown_body(blocks: &[Block]) -> String {
    blocks
        .iter()
...
// Scan parsed blocks: lexical (diction) over `lexical_text`, the per-sentence and
// run equations over the non-heading body, and the markdown shape tells. The two
// callers differ only in `lexical_text`: the reporting scan includes heading
// diction (a heading can read "Let's unpack…"), the score scans the body alone.
fn scan_blocks(blocks: &[Block], lexical_text: &str) -> Vec<Tell> {
    let body = markdown_body(blocks);
    let sents = sentences(&body);
    let mut out = Vec::new();
    lexical(lexical_text, &mut out);
    let mut metas = Vec::with_capacity(sents.len());
    for s in &sents {
        sentence_tells(s, &mut out);
        metas.push(sentence_meta(s));
    }
    run_tells(&metas, &mut out);
...
/// inline code are excluded, link URLs and image alt text are dropped (link visible
/// text kept), blockquote (quoted) text is excluded, and headings are scanned for
/// diction but their parallelism is not read as a run and they are not counted as
/// prose paragraphs.
...
/// Markdown-aware document `Score`. The score is computed over the non-heading body
/// only: heading diction stays an advisory tell in `scan_prose_markdown` but does
/// not escalate the density gate, so a clean body under buzzword headings is not
/// pushed over the threshold. Numerator and denominator share the body population.
pub fn tell_score_markdown(md: &str) -> Score {
    let blocks = parse_markdown(md);
    let body = markdown_body(&blocks);
    score_of(&scan_blocks(&blocks, &body), sentences(&body).len())
}
...
mod tests {
    ...
    #[test]
    fn tricolon_is_exactly_three_items() {
        assert!(ids("It is fast, clean, and simple.").contains(&"tricolon"));
        // four short items is an ordinary enumeration, not a tricolon
        assert!(!ids("It is fast, clean, neat, and simple.").contains(&"tricolon"));
    }

    #[test]
    fn repeated_ordinal_run_is_listicle_not_anaphora() {
        // the spec classifies a run by ordinal_led into one of the two; a repeated
        // ordinal opener is a listicle, never also an anaphora
        let got = ids("Next we eat. Next we sleep. Next we code.");
        assert!(got.contains(&"listicle"), "{got:?}");
        assert!(!got.contains(&"anaphora"), "{got:?}");
    }

    #[test]
    fn negative_parallelism_ignores_comparatives() {
        // "no faster than X" is a comparison, not antithesis
        assert!(!ids("It is no faster than light.").contains(&"negative-parallelism"));
    }

    #[test]
    fn markdown_excludes_blockquote_citations() {
        // a quoted block is a citation, scored as if authored would be a false positive
        assert!(md_ids("> It's not a tweak, it's a revolution.").is_empty());
        assert!(md_ids("> We delve into the rich tapestry to leverage synergy.").is_empty());
    }

    #[test]
    fn markdown_excludes_image_alt_text() {
        // descriptive alt text is not the author's prose
        assert!(md_ids("![delve into the tapestry](x.png)").is_empty());
    }

    #[test]
    fn markdown_loose_list_item_keeps_word_boundaries() {
        // two paragraphs in one list item must not fuse their boundary words
        let md = "- first line ending in delve\n\n  tapestry starts the second\n";
        assert!(md_ids(md).contains(&"ai-diction"), "{:?}", md_ids(md));
    }

    #[test]
    fn markdown_heading_diction_is_advisory_not_scored() {
        let md = "## Leveraging our robust ecosystem\n\n\
                  ## Harnessing the synergy paradigm\n\n\
                  ## A nuanced multifaceted tapestry\n\n\
                  ## Streamlining the landscape\n\n\
                  The code reads a file. It checks each line once.\n";
        // heading diction is still reported as an advisory tell
        assert!(scan_prose_markdown(md).iter().any(|t| t.id == "ai-diction"));
        // but a clean body is not pushed over the gate by buzzword headings
        assert!(!tell_score_markdown(md).over_threshold);
    }
}
